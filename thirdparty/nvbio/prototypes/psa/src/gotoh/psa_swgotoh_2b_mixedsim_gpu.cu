 ...
 * FILE: psa_swgotoh_2b_mixedsim_gpu
 * AUTHOR(S): Alejandro Chacon <alejandro.chacon@uab.es>
 * DESCRIPTION: Device functions for the SW-Gotoh GPU implementation using:
 *				(A) pack 4 SW in the same register emulating video instructions using just integer resources.
				...
				const uint32_t OPEN_INDEL_SCORE, const uint32_t EXTEND_INDEL_SCORE)
{
    ...
    for (uint32_t j = 1; j <= BAND_LEN; ++j)
    {
        ...
        const uint32_t   left     = E;
		      uint32_t   hi       = sim_vmaxu4(vmaxu4(left, top), diagonal);
		     	  	     hi       = sim_vmaxu4(hi, ZERO);

        H_diag                      = H_band[j];
        H_band[j]                   = hi;
        (* H_maxScore) = sim_vmaxu4((* H_maxScore), hi);
    }
