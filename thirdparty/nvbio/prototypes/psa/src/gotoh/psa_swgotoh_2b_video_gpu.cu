	...
	if (idCandidate < (candidatesNum))
	{
		...
		for(idColumn = 0; idColumn < numColumns; idColumn += BAND_LEN){
	        ...
	        for (uint32_t idBand = 0; idBand < BAND_LEN; ++idBand){
	            r_cache[idBand].c4 = make_char4(entryCandidate0 & 0x3,
	            							     entryCandidate1 & 0x3,
	            							     entryCandidate2 & 0x3,
	            							     entryCandidate3 & 0x3);
	        ...
	        for(idRow = 0; idRow < numRows; ++idRow){
				...
				update_band(idRow, q_i.i, (uint32_t *) r_cache, (uint32_t *) H_band, (uint32_t *) F_band, 
							(uint32_t *) H_temp, (uint32_t *) E_temp, &H_maxScore.i, 
