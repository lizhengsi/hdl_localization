...
__global__
void localProcessSWTiling(RAWHlfEntry_t *d_refHlfRaw, uint32_t *d_refPosition,
		   ...
		   uint32_t querySize, uint32_t candidateSize, uint32_t candidatesNum)
{
	...
	{
		const uint32_t   	 refPosition = d_refPosition[idCandidate];
		const RAWHlfEntry_t* candidate = d_refHlfRaw + (refPosition / RAW_BASES_PER_ENTRY);
		...
		uint32_t entryCandidate, actualEntryCandidate, lastEntryCandidate, idCandidateEntry = 0;
		...
		lastEntryCandidate = candidate[idCandidateEntry];
		...
		for(idColumn = 0; idColumn < numColumns; idColumn += BAND_LEN){
			...
			if((idColumn % RAW_BASES_PER_ENTRY) == 0){
				const uint32_t alignment = refPosition % RAW_BASES_PER_ENTRY;
				idCandidateEntry++;
				actualEntryCandidate = candidate[idCandidateEntry];
				entryCandidate =  actualEntryCandidate << (RAW_4B_LENGTH * (RAW_BASES_PER_ENTRY - alignment))
								| lastEntryCandidate >> (RAW_4B_LENGTH * alignment);
				lastEntryCandidate = actualEntryCandidate;
			}
...
psaError_t localProcessPairwiseReference(sequences_t* references, sequences_t *candidates, sequences_t *queries, alignments_t *alignments)
{
	...
	localProcessSWTiling<<<blocks, threads>>>(references->d_HlfRAW, candidates->d_refPosition,
